# ====================== Puxando dados da API do SIDRA - IPCA m/m ========================
library(sidrar)
library(zoo)
library(writexl)
library(dplyr)
...
print(unique(itens_7062$variavel_codigo))
print("Nomes únicos das variáveis:")
print(unique(itens_7062$variavel_nome))

# Criando mapeamento código-item para identificação
mapeamento_codigo_item <- itens_7062 %>%
...
ipca_variacoes <- itens_7062 %>%
  filter(variavel_codigo == "355") %>%
...
ipca_pesos <- itens_7062 %>%
  filter(variavel_codigo == "357") %>%
  select(data, variable, value) %>%
  mutate(data = ym(data)) %>%
  pivot_wider(
    names_from = variable,
...
# Criando tabela com dados do SIDRA e núcleos calculados
tabela_consolidada <- bind_rows(
  ipca_variacoes_transposta, # Dados do SIDRA primeiro
  nucleos_calculados # Núcleos calculados em seguida
)
...
write_xlsx(
  ...
  path = paste0("IPCA15s_", timestamp, ".xlsx")
)

print(paste("Arquivo exportado:", paste0("IPCAs_", timestamp, ".xlsx")))
print("Planilhas incluídas:")
print("- IPCA_Consolidado: Tabela única com SIDRA + núcleos calculados (variações mensais)")
